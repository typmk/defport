...
(deftest test-server-initiated-flag
  (testing "roots, sampling, elicitation-create, list_changed notifications are server→client"
    ...
    (is (spec/server-initiated? :notifications/tools/list_changed))
    (is (spec/server-initiated? :notifications/prompts/list_changed))
    (is (spec/server-initiated? :notifications/resources/list_changed))
