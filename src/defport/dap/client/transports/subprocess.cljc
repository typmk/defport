(ns defport.dap.client.transports.subprocess
  "Reference subprocess transport for the DAP client.

   Spawns an external debug adapter (debugpy, vscode-node-debug2,
   debugadapter-server, ...) via java.lang.ProcessBuilder on the
   JVM and reads/writes Content-Length framed JSON over its stdio.

   DAP and LSP share their wire framing exactly (Content-Length
   header + JSON body), so this namespace is structurally identical
   to defport.lsp.client.transports.subprocess. The only difference
   is which ClientTransport protocol the record extends.

   Optional namespace per CLAUDE.md principle 5 — defport.dap.client
   itself ships zero implementations.
   ...
   (def client
     (-> (sub/transport [\"python\" \"-m\" \"debugpy.adapter\"])
         (dap/create-client)
         (dap/connect! {:adapter-id \"debugpy\"})))
   ```

   The CLJS Node version is not yet shipped — see the LSP equivalent
   for tracking notes."
  (:require [defport.dap.client :as client]
...
#?(:clj
   (do
                         ...
                         (cond
                           ...
                           (let [chunk (byte-array n)]
                             ...
                             (let [[msgs new-decoder] (framing/feed decoder chunk)]
                               ...
                               (recur new-decoder)))))))))
           (.setDaemon true)
           (.setName "defport.dap.client.transports.subprocess/reader")
           (.start))))
                   ...
                   (loop []
                     (when @alive?*
                       (let [n (try (.read err buf 0 (alength buf))
                         ...
                         (when (>= n 0) (recur)))))))
           (.setDaemon true)
           (.setName "defport.dap.client.transports.subprocess/stderr")
           (.start))))

     (defrecord SubprocessDapTransport
     ...
     (defn transport
       "Construct a SubprocessDapTransport that runs `command` (a
        sequential of strings: program + arguments). Process is NOT
        started yet — defport.dap.client/connect! starts it.

        Example:
          (transport [\"python\" \"-m\" \"debugpy.adapter\"])
          (transport [\"node\" \"vscode-node-debug2.js\"])"
       [command]
       (->SubprocessDapTransport (vec command)
                                 (atom nil)
                                 (atom false)
                                 (LinkedBlockingQueue.)
                                 (Object.)))))
